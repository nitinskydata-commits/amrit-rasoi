...
exports.getAllCollaborations = async (req, res) => {
  try {
    ...
    // Non-admins see only their own collaboration
    if (!['admin', 'platform_admin'].includes(req.user.role)) {
      if (req.user.collaboration) {
        query._id = req.user.collaboration;
      } else {
        return res.status(200).json({ success: true, count: 0, collaborations: [] });
      }
    }
