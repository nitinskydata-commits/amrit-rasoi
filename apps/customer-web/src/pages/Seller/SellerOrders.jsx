...
const SellerOrders = () => {
  ...
  const fetchOrders = useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`${API_BASE_URL}/seller/orders`, {
  ...
  const handleUpdateStatus = async (orderId, newStatus, tracking = '') => {
    ...
    try {
      await axios.put(`${API_BASE_URL}/seller/order/${orderId}/item-status`, {
      ...
      alert(error.response?.data?.message || error.message || 'Error updating status.');
    } finally {
